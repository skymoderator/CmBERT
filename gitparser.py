            ...
            for commit in Repository(
                ...
                for mod in commit.modified_files:
                    ...
                    if (
                        added
                        and deleted
                        and len(added) + len(deleted) <= args.max_source_length - 3
                    ):
...
def main(args):
    repos = set(
        line.strip().replace("https://github.com/", "")
        for line in open(args.repositories, encoding="utf-8")
    )
    # with open(args.repositories, encoding="utf-8") as f:
    #     for idx, line in enumerate(f):
    #         line = line.strip().replace("https://github.com/", "")
    #         repos.add(line)
    ...
    with open(args.output_file, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            try:
                ...
                continue
    # with open(args.output_file, encoding="utf-8", mode='r') as f:
    #     data.extend([json.loads(line.strip()) for line in f if re.sub('^(?!(\{"msg")).*$', '', line.strip())])
    random.shuffle(data)
    n_data = len(data)
    write_jsonl(data[: int(n_data * 0.9)], path=args.output_dir, mode="train")
    write_jsonl(
        data[int(n_data * 0.9) : int(n_data * 0.95)], path=args.output_dir, mode="valid"
    )
    write_jsonl(data[int(n_data * 0.95) :], path=args.output_dir, mode="test")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="")
    parser.add_argument(
        "--repositories", type=str, required=True, help="repositories file path."
    )
    parser.add_argument(
        "--repos_dir",
        type=str,
