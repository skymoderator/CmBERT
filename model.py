    ...
    * `eos_id`- end of symbol ids in target for beam search.
    ...
    def __init__(
        ...
        self.attention_mask = nn.Transformer.generate_square_subsequent_mask(
            sz=2048, device=torch.device("cuda:0")
        )
    ...
    def forward(
        ...
        # source_ids: (bs, seq_len)
        # source_mask: (bs, seq_len)
        # target_ids: (bs, seq_len)
        # target_mask: (bs, seq_len)
        _, seq_len = source_ids.shape

        # BaseModelOutputWithPoolingAndCrossAttentions
        outputs = self.encoder(source_ids, attention_mask=source_mask)
        # outputs[0]=outputs.last_hidden_state: (bs, seq_len, hidden_size=768)
        # permute: (seq_len, bs, hidden_size)
        encoder_output = outputs[0].permute([1, 0, 2]).contiguous()
        if target_ids is not None:
            # (seq_len, seq_len)
            # upper triangle filled with -1e4 (exclude diagonal line), others zeros
            # attn_mask = -1e4 * (
            #     1 - self.bias[: target_ids.shape[1], : target_ids.shape[1]]
            # )
            # embedding: (bs, seq_len, hidden_size)
            # permute: (seq_len, bs, hidden_size)
            tgt_embeddings = (
            ...
            out = self.decoder(
                ...
                # tgt_mask=attn_mask,
                tgt_mask=self.attention_mask[:seq_len, :seq_len],
                tgt_is_causal=True,
                memory_key_padding_mask=(1 - source_mask).bool(),
            )
            # dense: (seq_len, bs, hidden_size)
            # permute: (bs, seq_len, hidden_size)
            hidden_states = torch.tanh(self.dense(out)).permute([1, 0, 2]).contiguous()
            # (bs, seq_len, vocab_size)
            lm_logits = self.lm_head(hidden_states)

            # Shift so that tokens < n predict n
            # label: ___ <1> <2> <3> <4> <0> <0> <0> </s>
            # logit: <a> <b> <c> <d> <e> <f> <g> <h> ___
            # masks: ___ <1> <1> <1> <1> <0> <0> <0> <1>
            # (bs * (seq_len - 1))
